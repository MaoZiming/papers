...
* **Priority donation / inheritance**
  ...
  * Here's a simple example:
    * **Low-Priority Task (L)** owns a mutex.
    * **High-Priority Task (H)** tries to acquire the same mutex but is blocked because it's owned by **L**.
    * **Medium-Priority Task (M)** preempts **L** because **M** has a higher priority than **L** but lower than **H**.
    ...
    * When **H** is blocked by **L**, the system temporarily elevates the priority of **L** to that of **H**.
    * Now, **L** can preempt **M** because it has "inherited" the higher priority.
    * **L** quickly finishes its work, releases the mutex.
    * **H** can now acquire the mutex and proceed, and **L** returns to its original low priority.
  * Control parameters
